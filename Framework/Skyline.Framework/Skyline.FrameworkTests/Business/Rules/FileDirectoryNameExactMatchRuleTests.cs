...
namespace Skyline.Framework.Business.Rules.Tests
{
	...
	public class FileDirectoryNameExactMatchRuleTests
	{
		...
		{
			var rule = new FileDirectoryNameExactMatchRule { Criteria = AppDomain.CurrentDomain.BaseDirectory + @"\mustfail\" };
		...
		{
			var rule = new FileDirectoryNameExactMatchRule { Criteria = AppDomain.CurrentDomain.BaseDirectory };
			var fileInfo = new FileInfo(AppDomain.CurrentDomain.BaseDirectory + @"\testfile.txt");
